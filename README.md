...
- 两种悬浮窗模式：
  ...
  - **吸附 Codex 窗口**：在 Codex 工具栏水平居中并保持在 Codex 上一层；Codex 失焦后继续显示，但可被其他应用正常遮挡。Codex 最小化或窗口丢失时隐藏。
- 托盘图标左键、右键均可打开菜单。
- 托盘菜单显示当前连接状态，并提供：
  - 显示或隐藏悬浮窗。
  - 独立展示或吸附 Codex。
...
- 默认跟随 `config.toml` 中已有的 `language`、`locale` 或 `uiLanguage`；没有语言字段时，中文族系统语言使用中文，其他语言使用英语。程序只内置中文和英语，其他翻译需自行维护。
